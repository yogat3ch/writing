...
### Abuse Prevention
A problematic aspect of an pure democracy voting system such as this is the potential for abuse. The spread of misinformation for manipulative purposes has been identified as a 
...
The questions are derived from the reader highlights added during the refinement phase. All sections with highlighting are translated to questions by AI, with a point score attributed to each question based on the aggregated number of highlights made on that section. The score is used as a weighting system that is algorithmically balanced with a randomization system to generate a set of questions proportional to the number of critical aspects of the legislation for each prospective voter. In order to vote on a piece of legislation. 
...
MDQ2NjU3NTc0ODR9fSwiaGlzdG9yeSI6WzIwMDQxOTYwNTFdfQ
==
-->